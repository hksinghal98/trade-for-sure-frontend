...
const handleBuyOrSell = (action) => {
  if (!brokerName4 || !exchange || !selectsymbol || !instrument) {
    ...
    {tableDatafetch.map((row) => (
      <tr key={row.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
        {Object.values(row).map((value, index) => (
          <td key={index} scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-slate-800">
            {value}
          </td>
        ))}

        <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-slate-800">
        ...
        <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-slate-800">
          <Button className="bg-red-600/90 text-white hover:bg-red-700"
          onClick={() => handleBuyOrSell("Sell")}
          >
            Sell</Button>
        </td>
        <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-slate-800">
          <Button className="bg-red-700 text-white hover:bg-red-700" onClick={() => handleDelete(row.id)}>Delete</Button>    
        </td>
      </tr>
    ))}
  </tbody>
